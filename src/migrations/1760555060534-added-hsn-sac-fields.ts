...
export class AddedHsnSacFields1760555060534 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query(
      "ALTER TABLE `manual_payment_config` CHANGE `enabled` `enabled` tinyint NOT NULL DEFAULT 1",
      undefined,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<any> {
    await queryRunner.query(
      "ALTER TABLE `manual_payment_config` CHANGE `enabled` `enabled` tinyint NULL",
      undefined,
    );
    await queryRunner.query(
